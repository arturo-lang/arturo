...
proc Core_And*[F,X,V](f: F, xl: X): V =
...
proc Core_Get*[F,X,V](f: F, xl: X): V =
    let v = f.validate(xl)
...
proc Core_Loop*[F,X,V](f: F, xl: X): V =
    let v = f.validate(xl)
