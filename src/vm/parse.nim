...
template parseString(p: var Parser, stopper: char = Quote) =
    ...
    while true:
        case p.buf[pos]:
            of EOF: 
                Error_UnterminatedString("", initialLine)
            of stopper:
                inc(pos)
                break
            of '|':
                ...
                if regexString:
                    if p.buf[pos+1]==RCurly:
                        ...
                        while p.buf[pos] in {'i','m','s'}:
                            ...
                            inc(pos)
                        if p.buf[pos] != RCurly:
                            Error_UnterminatedString("regex", initialLine)
                        else:
                            inc(pos)
                            break
                    else:
...
template parseSafeString(p: var Parser) =
    ...
    while true:
        case p.buf[pos]:
            of EOF: 
                Error_UnterminatedString("", p.lineNumber)
                break
            of CR:
                pos = lexbase.handleCR(p, pos)
                when not defined(windows):
    ...
    while true:
        ...
        case p.buf[p.bufpos]
            of EOF:
                if unlikely(level!=0): 
                    if isSubBlock:
                        Error_MissingClosingSquareBracket(initialLine)
                    else:
                        Error_MissingClosingParenthesis(initialLine)
                break
            of Quote:
                parseString(p)
                if unlikely(p.buf[p.bufpos] == Colon):
            ...
            of RBracket:
                if isSubBlock:
                    ...
                    if isSubInline:
                        ...
                        Error_StrayClosingSquareBracket(p.lineNumber)
            of LParen:
                inc(p.bufpos)
            ...
            of RParen:
                if isSubInline:
                    ...
                    if isSubBlock:
                        ...
                        Error_StrayClosingParenthesis(p.lineNumber)
            of LCurly:
                parseCurlyString(p)
            of RCurly:
                Error_StrayClosingCurlyBracket(p.lineNumber)
            of '\194':
