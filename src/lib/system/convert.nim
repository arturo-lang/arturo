...
proc Convert_toNumber*[F,X,V](f: F, xl: X): V {.inline.} =
    ...
    case v[0].kind
        of SV:
            if unlikely(S(0).startsWith("0x")): 
                var ret = 0
                discard parseHex(S(0),ret)
                result = INT(ret)
            elif unlikely(S(0).startsWith("0b")):
                var ret = 0
                discard parseBin(S(0),ret)
                result = INT(ret)
            elif unlikely(S(0).startsWith("0o")):
                var ret = 0
                discard parseOct(S(0),ret)
                result = INT(ret)
            else:
                if S(0).contains("."):
                    var ret = 0.0
