...
/*
class Database__Table : Func {
	this(string ns="") { super(ns ~ "table","create new SQLite table using name and fields",[[sV,dV]],[dV]); }
	override Value execute(Expressions ex, string hId=null) {
		Value[] v = validate(ex);
		alias title = S!(v,0);
		Value fields = v[1];

		writeln("creating new table");
		Value obj = new Value(fields);

		obj[_ID] = new Value(getUUID());
		obj[_TYPE] = new Value("table");
