...
## Full per-module audit (2026-05-02)

Goal: every builtin with args ends up using `dispatch` / `dispatchWithLiteral`,
even single-kind 1-clause forms. `NoArgs` builtins are out of scope. This
section is the source of truth for what's left; the older "Conversion targets"
and "Roadmap ahead" sections below are kept for history but are superseded
where they disagree.

Status legend: **D** = done, **T** = todo (with proposed form), **X** = deferred
...
- `split` — X — Phase D, heavy attrs marquee target.
- `contains?`, `in?`, `key?` — X — Object magic (Phase E).

### Sets.nim
All DONE: `difference`, `intersection`, `powerset`, `union`, `disjoint?`, `intersect?`, `subset?`, `superset?`.

### Statistics.nim
All DONE: `average`, `deviation`, `kurtosis`, `median`, `skewness`, `variance`.
...
### Colors.nim
All DONE: `blend`, `darken`, `desaturate`, `grayscale`, `invert`, `lighten`, `palette`, `saturate`, `spin`.
...
### Bitwise.nim
- `nand`, `nor`, `shl`, `xnor` — D.
- `and`, `or`, `xor`, `not`, `shr` — M — `generateOperationA` / `generateOperationB`; **already kind-dispatching macros**, do NOT re-express via `dispatch`. Reconciliation with `dispatch` is a separate refactor (covered in old roadmap).

### Arithmetic.nim
- `add`, `dec`, `div`, `divmod`, `fdiv`, `inc`, `mod`, `mul`, `neg`, `pow`, `sub` — M — `generateOperationA` / `generateOperationB` family. Same as Bitwise: deferred to separate refactor.
